					...
					<div class="section clear">
						<c:forEach var="item" items="${result}" varStatus="status">
							<div class="item">
								...
								<div class="overlay abs">
									<c:choose>
										<c:when test="${!empty webUser}">
											<i><a target="_blank" onclick="winOpen('${ctx}/forwardGame?gameType=bbin&gameid=${item.eleGameId}',window.screen.width,window.screen.height,0,0,'game','bbin');return false;" href="javascript:void(0);">开始游戏</a>
											</i>
										</c:when>
										<c:otherwise>
								...
								<a class="gm-ic-mg lazy" data-original="${resourceRoot}/web/ybb/common/images/bbin/${item.eleGamePic}" style="background-image: url(${resourceRoot}/web/ybb/common/images/grey.gif);background-size: 100%; background-position: 0px -6px;"></a>
							</div>
						</c:forEach>
					</div>
				</div>
