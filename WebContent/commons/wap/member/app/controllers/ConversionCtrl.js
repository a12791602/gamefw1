angular.module('AppController')
	.controller('ConversionController', ['$rootScope', '$scope', '$filter', 'ngDialog', 'CommomService',
		...
		$scope.conversionIn = function (type) {
			var typeLabel = type.toUpperCase().replace("NEW", "");
			$scope.dialog.title = typeLabel + "平台转入";
			$scope.dialog.subtitle = "从总平台转入到" + typeLabel + "平台";
		...
		$scope.conversionOut = function (type) {
			var typeLabel = type.toUpperCase().replace("NEW", "");
			$scope.dialog.title = typeLabel + "平台转出";
			$scope.dialog.subtitle = "从" + typeLabel + "平台转出到总平台";
			$scope.dialog.type = type;
			ngDialog.open({
