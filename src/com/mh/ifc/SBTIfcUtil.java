...
public class SBTIfcUtil {
	 ...
	 * SbtResBean
	 */
	public static SbtResBean queryOrderStatus(Map<String, String> params){
		SbtResBean result = null;
		try {
			logger.info("SBT【查询订单】开始");
			
			String respJson = HttpClientUtil.post(url + "/sbt/agent/api/queryOrderStatus", params);
			JSONObject json =JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), SbtResBean.class);
			}
			logger.info("SBT【查询订单】结束");
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("SBT【查询订单】失败",e);
		}
		return result;
	}
