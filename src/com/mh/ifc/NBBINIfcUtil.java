...
private static final Log logger = LogFactory.getLog(NBBINIfcUtil.class);
	...
	public static NBbinResResult login(Map<String, String> params) throws Exception {
		...
		try {
			...
			logger.info("BBIN【登录账号】结束");
		} catch (Exception e) {
			logger.info("BBIN【登录账号】失败",e);
		}
		return result;
	}
	 ...
	 * @throws Exception
	 */
	public static NBbinResResult playGameMethod(Map<String, String> params) throws Exception {
		NBbinResResult result = null;
		try {
			logger.info("BBIN【登录账号】开始");
			
			String respJson = HttpClientUtil.post(url + "/nbbin/agent/api/playGameMethod", params);
			JSONObject json = JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), NBbinResResult.class);
	 ...
	 * @throws Exception
	 */
	public static NBbinResResult playGameByH5Method(Map<String, String> params) throws Exception {
		NBbinResResult result = null;
		try {
			logger.info("BBIN【登录账号】开始");
			
			String respJson = HttpClientUtil.post(url + "/nbbin/agent/api/playGameByH5Method", params);
			JSONObject json = JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), NBbinResResult.class);
	...
	public static NBbinResResult register(Map<String, String> params){
		...
		try {
			...
			logger.info("BBIN【注册账号】结束");
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("BBIN【注册账号】失败",e);
		}
		return result;
	}
	...
	public static NBbinResResult balance(Map<String, String> params) {
		...
		try {
			...
			logger.info("查詢BBIN余额接口结束");
		} catch (Exception e) {
			logger.info("查詢BBIN余额接口失败",e);
		}
		return result;
	}
	 ...
	 * AgResResult
	 */
	public static NBbinResResult queryOrderStatus(Map<String, String> params) throws Exception {
		NBbinResResult result = null;
		try {
			logger.info("BBIN【查询订单状态】接口开始");
			String respJson = HttpClientUtil.post(url + "/nbbin/agent/api/checkTransfer", params);
			JSONObject json = JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), NBbinResResult.class);
			}
			logger.info("BBIN【查询订单状态】接口结束");
		} catch (Exception e) {
			throw e;
		}
		return result;
