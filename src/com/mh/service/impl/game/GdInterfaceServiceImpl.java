 ...
 *
 */
@Service("gdInterfaceServiceImpl")
public class GdInterfaceServiceImpl extends BaseFlatInfo implements FlatInterfaceService {
	...
	public String login(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			params.put(GdConts.USERID, webUserFlat.getGdUserName());// 游戏帐号
			GdResResult result = GDIfcUtil.login(params);
			if (result != null) {
		...
		if(StringUtils.isEmpty(webUserFlat.getGdUserName())){
			 try {
				 ...
				 if(webUserFlat.getGdStatus()!= null && webUserFlat.getGdStatus().intValue()==1){
					 ...
					 tsMsg = updateGdAccountPonit(webUser, flatName, webUserFlat.getGdUserName(), optType, points, eduForward, eduForwardRemark);
				 }else{
	...
	private String updateGdAccountPonit(WebUser webUser,String flatName,String flatAccount,String optType,int points,int eduForward, String eduForwardRemark) {
		...
		if (StringUtils.equalsIgnoreCase(optType, WebConstants.EDU_TYPE_2)){// 转入 （主帐号-》游戏平台）
			return GdDepositOpt(webUser,flatName,optType, flatAccount, points,eduForward, eduForwardRemark);
		}
	...
	private String GdDepositOpt(WebUser webUser,String flatName,String optType, String flatAccount, int points, int eduForward, String eduForwardRemark) {
