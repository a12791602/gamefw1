...
public class WebRecordsDao extends BaseDao {
	...
	public Page findFinancePage(Page page, WebRecords records) {
		...
		if ("eduHistory".equals(records.getCode())) {
			sql = " SELECT flat_name,edu_order,user_name,edu_points,edu_type,edu_status,DATE_FORMAT(create_time,'%Y-%m-%d %H:%i:%s') AS create_time,edu_forward_remark from  t_web_edu where 1=1 ";
	...
	public Page findRecordsPage(Page page, WebRecords records) {
		...
		} else if (WebConstants.FLAT_NAME_NEWNT.equals(records.getCode()) || WebConstants.FLAT_NAME_NT.equals(records.getCode())) {
	...
	public Page findEleRecordPage(Page page, WebRecords records) {
		List<Object> list = new ArrayList<Object>();
		if(StringUtils.equals(records.getBetSportType(), WebConstants.FLAT_NAME_NEWNT) || StringUtils.equals(records.getBetSportType(), WebConstants.FLAT_NAME_NT)){
			records.setBetSportType(WebConstants.FLAT_NAME_NT);
		}
