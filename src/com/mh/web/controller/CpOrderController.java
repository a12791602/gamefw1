...
//		}
	...
	public void saveOrder(HttpServletRequest request,HttpServletResponse response) throws Exception{
		try{
			...
			if(uc==null){
				ResponsePrintUtils.outPrintMsg(request, response, "请登录后在下单!");
				return;
			}
			boolean flag = this.webUserService.checkFlatPermissions(CommonConstant.PLAT_PARAM_CAIPIAO,uc.getUserName());
			if(flag){
				ResponsePrintUtils.outPrintMsg(request, response, "无权限访问该平台，请联系我们处理！");
				return;
			}
			
			if(StringUtils.isEmpty(gameCode) || StringUtils.isEmpty(jsonData) || StringUtils.isEmpty(orderFlag)){
				ResponsePrintUtils.outPrintMsg(request, response, "请求参数为空!");
				return;
			}
			if(!this.validateCpOpenTime(gameCode)){
				ResponsePrintUtils.outPrintMsg(request, response, "该期已关盘!");
				return;
			}
			...
			ResponsePrintUtils.outPrintMsg(request, response,"成功提交注单！");
		}catch(RuntimeException e){
			e.printStackTrace();
			logger.error("提交注单失败!",e);
			ResponsePrintUtils.outPrintMsg(request, response, "提交注单失败！");
		}
	...
	public boolean validateBetEdu(HttpServletRequest request,HttpServletResponse response){
		...
		CpTomResults tomResults = this.cpResultsDao.getNextResults(gameCode);//下一期排期
		int minSingle=0;
		int maxSingle=0;
		...
		if(CommonConstant.BJKL8_CODE_PARAM.equals(gameCode) || CommonConstant.CAKENO_CODE_PARAM.equals(gameCode)){
			...
			records.setQs(tomResults.getQs());
			...
			if(reFlag){
				ResponsePrintUtils.outPrintMsg(request, response, "禁止逆向下注");
				return false;
			}
		...
		for(int i=0;i<jsonArray.size();i++){
			...
			if(minSingle > xzje || maxSingle < xzje){
				ResponsePrintUtils.outPrintMsg(request, response, "下注金额最低:" + minSingle + "元,最高:" + maxSingle + "元");
				return false;
			}
		}
		
		if((int)cpGame.getIsEnable() == (int)CpCommonConstant.IS_ENABLE){//大盘口启用状态
			ResponsePrintUtils.outPrintMsg(request, response, cpGame.getGameTypeName() + "已进入暂时维护状态,抱歉!");
			return false;
		}else{//小盘口启用状态
			if((int)cpType.getIsEnable() == (int)CpCommonConstant.IS_ENABLE){
				ResponsePrintUtils.outPrintMsg(request, response, cpType.getCpTypeName() + "已进入暂时维护状态,抱歉!");
				return false;
			}
		}
		
		if(!flag){//常规注单
			ResponsePrintUtils.outPrintMsg(request, response, normalMsg);
			return false;
		}
		...
		if(!StringUtils.equalsIgnoreCase(orderFlag, "normal")){//非 常规订单 验证盘口限额
			...
			if(xzje > singleCredit){
				ResponsePrintUtils.outPrintMsg(request, response, "当期最大限额:" + singleCredit + "元");
				return false;
			}
			...
			if((xzje + userSingleCredit) > singleCredit){
				ResponsePrintUtils.outPrintMsg(request, response, "当期最大限额:" + singleCredit + "元！您下注累积金额为:" + (xzje + userSingleCredit) + "元");
				return false;
			}
		}
		...
		if(moneyTotal>userMoney){
			ResponsePrintUtils.outPrintMsg(request, response, "余额不足!");
			return false;
		}
	 ...
	 * boolean
	 */
	public boolean validateCpOpenTime(String gameCode){
		CpTomResults nextResult = cpResultsService.getNextResults(gameCode);
		Date currDate = new Date();
		Date kjsjDate = null;
		try {
			kjsjDate = DateUtil.parse(nextResult.getGtKjsj(), "yyyy-MM-dd HH:mm:ss");
		...
		boolean eFlag = CpCacheUtil.isExistsOpenResults(gameCode, nextResult.getFormatQs());
		if(eFlag){
			return false;
		}
	 ...
	 * @param config
	 */
	/*private void validBetMoney(double xzje, CpConfig config) {
		Integer minSingle = config.getGminSingle();//最低下注金额
		Integer maxSingle = config.getGmaxSingle();//最高下注金额
		if(null != minSingle && null != maxSingle){
			if(minSingle > xzje || maxSingle < xzje){
				throw new RuntimeException("号码:"+ config.getNumber() +"下注金额最低:" + minSingle + "元,最高:" + maxSingle + "元");
			}
		}else{
			CpType type = CpConfigCache.TYPE_OBJ_CACHE_MAP.get(config.getBigtypeCode()+"_"+config.getCpTypeCode());
			minSingle = type.getGminSingle();
			maxSingle = type.getGmaxSingle();
			if(null != minSingle && null != maxSingle){
				if(minSingle > xzje || maxSingle < xzje){
					throw new RuntimeException(type.getBigtypeCode() + "_" + type.getCpTypeName() +"下注金额最低:" + minSingle + "元,最高:" + maxSingle + "元");
				}
			}else{
				throw new RuntimeException(type.getBigtypeCode() + "_" + type.getCpTypeName() + "未配置限额信息");
			}
		}
	}*/
	 ...
	 * void
	 */
	@RequestMapping("/getCpOrderRecord/{gameid}")
	public void getCpOrderRecord(@PathVariable(value="gameid") Integer gameid,HttpServletRequest request,HttpServletResponse response){
		try {
			List<Map<String,Object>> list = null;
			UserContext uc = this.getUserContext(request);
			if (uc != null && uc.getUserName() != null) {
				CpGame cpGame = CpConfigCache.GAME_CACHE_MAP.get(gameid);
				list = this.cpOrderService.getOrderResult8List(cpGame.getGameTypeCode(),uc.getUserName());
			}
 
			ResponsePrintUtils.outPrintMsg(request, response,JSON.toJSONString(list));
		} catch (Exception e) {
			e.printStackTrace();
			ResponsePrintUtils.outPrintMsg(request, response, "查询订单失败！");
		}
	}
}
