...
import com.mh.web.util.PayUtil;
...
//			bankHk.setPayDama(NumberUtils.toInt(extra_return_param.split("_")[1], 0));// 是否是打码量再次入款
	 ...
	 * ModelAndView
	 */
	@RequestMapping("/payReturnHxwxPage")
	public ModelAndView payReturnHxwxPage(HttpServletRequest request, HttpServletResponse response){
		String paymentResult=request.getParameter("paymentResult");
	 
		logger.info("支付返回报文::"+paymentResult);
		Map<String,Object> maps=PayUtil.getHxWxCallMap(paymentResult);

		Map<String,String> headMap=(Map<String,String>)maps.get("headMap");
 
		String succ = headMap.get("Status");
		
		logger.info("环讯微信页面通知结果:"+succ);
		String message="";
