...
<div class="ybb-main ybb-live-main">
	...
	<div class="ui-mod-item">
		<c:choose>
			<c:when test="${empty webUser }">
			<a href="javascript:alert('请先登录');void(0);" class="ui-btn ui-btn-f" data-ajax="false">
				<img src="${resourceRoot }/m/img/live_og.png" alt="" />
			</a>
			</c:when>
			<c:otherwise>
			<a href="javascript:void(0);" onclick="checkFlatSatus('${ctx}/m/game/forwardGame?gameType=og','og');" class="ui-btn ui-btn-f" data-ajax="false">
				<img src="${resourceRoot }/m/img/live_og.png" alt="" />
			</a>
			</c:otherwise>
		</c:choose>
