				...
				<div class="ybb-slot-main">
					<div data-role="navbar" class="slot-hall">
						<ul>
							...
							<li><a href="${ctx }/m/main?code=slot_pt" data-ajax="false" class="ui-btn-active">PT电子</a></li>
							...
							<c:forEach var="item" items="${map.value }" varStatus="itemStatus">
								<c:if test="${itemStatus.index % 2 == 0 }">
									<c:choose>
										<c:when test="${empty webUser}">
											<div class="ui-block-a item">
												<a href="javascript:void(0);" onclick="alert('请先登录');">
													<div class="r1" style=" background-image: url(${resourceRoot}/m/img/pt/${item.eleGameCode}.png);height: 150px;"></div>
										...
										<c:otherwise>
											<div class="ui-block-a item">
												<%--<a href="javascript:void(0);" onclick="checkFlatSatus('${ctx }/m/pt/goPt?gameName=${item.eleGameCode}','pt');" data-ajax="false">
													<div class="r1" style=" background-image: url(${resourceRoot}/m/img/pt/${item.eleGameCode}.png);height: 150px;"></div>
													<p class="r2 ac">${item.eleGameCnname}</p>
												</a>--%>
												<a href="javascript:void(0);" onclick="checkFlatSatus('${ctx }/m/game/forwardGame?gameName=${item.eleGameCode}&gameType=pt','pt');" data-ajax="false">
													<div class="r1" style=" background-image: url(${resourceRoot}/m/img/pt/${item.eleGameCode}.png);height: 150px;"></div>
													<p class="r2 ac">${item.eleGameCnname}</p>
												</a>
											</div>
									...
									<c:choose>
										...
										<c:otherwise>
											<div class="ui-block-b item">
												<%--<a href="javascript:void(0);" onclick="checkFlatSatus('${ctx }/m/pt/goPt?gameName=${item.eleGameCode}','pt');" data-ajax="false">
													<div class="r1"
														style=" background-image: url(${resourceRoot}/m/img/pt/${item.eleGameCode}.png);height: 150px;"></div>
													<p class="r2 ac">${item.eleGameCnname}</p>
												</a>--%>
												<a href="javascript:void(0);" onclick="checkFlatSatus('${ctx }/m/game/forwardGame?gameName=${item.eleGameCode}&gameType=pt','pt');" data-ajax="false">
													<div class="r1"
