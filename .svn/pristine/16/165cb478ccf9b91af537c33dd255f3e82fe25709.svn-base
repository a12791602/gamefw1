...
public class AGIfcUtil {
	...
	public static AgResResult login(Map<String, String> params) throws Exception {
		...
		try {
			...
			logger.info("AG【登录账号】失败", e);
		}
		return result;
	}

	public static AgResResult callapp(Map<String, String> params) throws Exception {
		AgResResult result = null;
		try {
			logger.info("AG【获取启动客户端URL】开始");

			String respJson = HttpClientUtil.post(url + "/ag/agent/api/callapp", params);
			JSONObject json = JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), AgResResult.class);
			}
			logger.info("AG【获取启动客户端URL】结束");
		} catch (Exception e) {
			logger.info("AG【获取启动客户端URL】失败", e);
		}
		return result;
	}
	 ...
	 * AgResResult
	 */
	public static AgResResult queryOrderStatus(Map<String, String> params) throws Exception {
		AgResResult result = null;
		try {
			logger.info("AG【查询订单状态】接口开始");
			String respJson = HttpClientUtil.post(url + "/ag/agent/api/queryOrderStatus", params);
			JSONObject json = JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), AgResResult.class);
			}
			logger.info("AG【查询订单状态】接口结束");
		} catch (Exception e) {
			throw e;
		}
		return result;
