							...
							<div class="sheet-content mt20">
								<table class="center">
									...
									<c:if test="${empty page.result}">
										<tr height="125px;">
											<td colspan="5"><span class="gray-dark">查询结果 - 暂无纪录</span></td>
										</tr>
									</c:if>
									<c:forEach var="item" items="${page.result}" varStatus="item_index">
										<tr class="sheet-body s12">
											...
											<td align="center">${item.bet_usr_win<0?"<font color=red>":"<font color=green>"} ${item.bet_usr_win}</td>
										</tr>
									</c:forEach>
									<c:if test="${!empty page.result}">
