								...
								<table class="center">
									...
									<c:forEach var="item" items="${page.result}" varStatus="item_index">
										<tr class="sheet-body s12">
											<td align="center"><fmt:formatDate value="${item.bet_time}" pattern="yyyy-MM-dd HH:mm:ss" /></td>
											<td align="center">${item.bet_Wagers_id}</td>
											<td align="center">${item.bet_game_table}</td>
											<td align="center">${item.bet_content}</td>
											<td align="center">
											<c:if test="${item.bet_game_result eq 'Bet' }">
												Bet ${item.bet_in }
											</c:if>
											<c:if test="${item.bet_game_result eq 'Win' }">
												Win ${item.bet_usr_win }
											</c:if>
											</td>
										</tr>
									</c:forEach>
									<c:if test="${!empty page.result}">
										<tr class="sheet-title">
											<td colspan="5" class="pr20 ar"><span class="right">总计${page.totalCount}笔记录&nbsp;&nbsp;共${page.totalPages}页&nbsp;&nbsp;当前第${page.pageNo }页 &nbsp;&nbsp;</span></td>
										</tr>
									</c:if>
								</table>
							</div>
