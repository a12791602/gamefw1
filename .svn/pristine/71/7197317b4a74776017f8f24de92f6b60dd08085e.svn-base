...
public class SaIfcUtil {
	 ...
	 * SbtResBean
	 */
	public static SaResBean queryOrderStatus(Map<String, String> params){
		SaResBean result = null;
		try {
			logger.info("SA【查询订单】开始");
			
			String respJson = HttpClientUtil.post(url + "/sa/agent/api/queryOrderStatus", params);
			JSONObject json =JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), SaResBean.class);
			}
			logger.info("SA【查询订单】结束");
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("SA【查询订单】失败",e);
		}
		return result;
	}
