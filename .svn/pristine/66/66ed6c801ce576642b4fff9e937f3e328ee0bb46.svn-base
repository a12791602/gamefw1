...
public class NewPTIfcUtil {
	...
	public static NewPtResResult register(Map<String, String> params) {
		...
		try {
			...
			e.printStackTrace();
			logger.info("PT【注册账号】失败", e);
		}
		return ptResResult;
	}

	
	public static NewPtResResult queryOrderStatus(Map<String, String> params){
		NewPtResResult ptResResult = null;
		try {
			logger.info("PT【查询订单】开始");
			
			String respJson = HttpClientUtil.post(url + "/newpt/agent/api/queryOrderStatus", params);
			logger.info("PT【查询订单】返回报文："+respJson);
			JSONObject json =JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				ptResResult = JSON.parseObject(json.getString("msInfo"), NewPtResResult.class);
			}
			logger.info("PT【查询订单】结束");
		} catch (Exception e) {
			e.printStackTrace();
			logger.info("PT【查询订单】失败",e);
		}
		return ptResResult;
	}
		...
		if (result != null) {// 成功
			if ("000000".equals(result.getCode())) {// 成功
				...
				if ("19".equals(result.getCode())) {// 帐号已经存在
					...
					// System.out.println("############" + webUserFlag + tmpMgUserName);
					registerParams.put(PtConts.USER_ID, webUserFlag + tmpMgUserName);
