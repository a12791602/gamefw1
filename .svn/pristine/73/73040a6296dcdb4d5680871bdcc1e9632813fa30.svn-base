...
public class DSIfcUtil {
	 ...
	 * DsResResult
	 */
	public static DsResResult queryOrderStatus(Map<String, String> params) throws Exception {
		DsResResult result = null;
		try {
			logger.info("DS【查询订单】接口开始");
			String respJson = HttpClientUtil.post(url + "/ds/agent/api/checkref", params);
			JSONObject json =JSON.parseObject(respJson);
			if (StringUtils.equals("000000", json.getString("code"))) {
				JSONObject msInfo = json.getJSONObject("msInfo");
				result = JSON.parseObject(msInfo.toString(), DsResResult.class);
			}
			logger.info("DS【查询订单】接口结束");
		} catch (Exception e) {
			throw e;
		}
		return result;
