...
public class NewNtInterfaceServiceImpl extends BaseFlatInfo implements FlatInterfaceService {
	...
	public ModelAndView mobileLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		String domain = this.getWebDomain(request) + "m/main?code=slot_nt";
		try {
			WebUserFlat webUserFlat = webUserFlatService.getWebUserFlat(uc.getUserName());
			if (StringUtils.isEmpty(webUserFlat.getNewNtUserName())) {
				webUserFlat = NewNtIfcUtil.registNewNtAccout(webUserFlat);
				if (webUserFlat.getNewNtUserName() != null && webUserFlat.getNewNtStatus().intValue() == 1) {
					...
					sendErrorMsg(response,"【NT平台注册用户失败】",domain);
					return null;
				}
			}
			Map<String, String> params = new HashMap<String, String>();
			...
			params.put(Conts.USER_AGENT, webUserFlat.getUserAgent());
			params.put(Conts.WEB_USER_NAME, webUserFlat.getUserName());
			result = NewNtIfcUtil.login(params);
			if (result == null) {
				sendErrorMsg(response, "【NT】平台登录失败", domain);
				return null;
			}
	...
	private String updateNewNtAccountPonit(WebUser webUser, String flatName, String flatAccount, String optType, int points, int eduForward, String eduForwardRemark)
			 {
		...
		if (StringUtils.equalsIgnoreCase(optType, WebConstants.EDU_TYPE_2)) {// 转入 （主帐号-》游戏平台）
			return newNtDepositOpt(webUser, flatName, optType, flatAccount, points, eduForward, eduForwardRemark);
		}
	...
	private String newNtDepositOpt(WebUser webUser, String flatName, String optType, String flatAccount, int points, int eduForward, String eduForwardRemark)  {
