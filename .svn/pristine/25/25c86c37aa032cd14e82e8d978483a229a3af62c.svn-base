								...
								<div class="overlay abs">
									<c:choose>
										<c:when test="${!empty webUser}">
											<i><a target="_blank" onclick="winOpen('${ctx}/forwardGame?gameType=gd&game=${item.eleGameId}',window.screen.width,window.screen.height,0,0,'game','gd');return false;" href="javascript:void(0);">开始游戏</a>
											</i>
										</c:when>
										<c:otherwise>
								...
								<c:if test="${gameType1 eq 'Pot' }">
						          ...
						          <c:choose>
						            <c:when test="${gameType1 eq 'Pot' }">
						              <img class="lazy" width="230" height="150" data-original="${resourceRoot}/web/ybb/common/images/gd/${item.eleGamePic}" src="${resourceRoot}/web/ybb/common/images/grey.gif" />
						            </c:when>
						            <c:otherwise>
						              <a class="gm-ic-mg lazy" data-original="${resourceRoot}/web/ybb/common/images/gd/${item.eleGamePic}" style="background-image: url(${resourceRoot}/web/ybb/common/images/grey.gif);background-size: 100%; background-position: 0px -6px;"></a>
						            </c:otherwise>
						          </c:choose>
							</div>
						</c:forEach>
